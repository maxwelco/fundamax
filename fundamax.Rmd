...
#empresas <- read_sheet("https://docs.google.com/spreadsheets/d/1vtts8WdUYYVqi9ygLlIuWzS_ISefoR4WDLmC2E3pXCw/edit#gid=0") %>% 
 # arrange(`SETOR ECONOMICO`, SUBSETOR) 
...
inputPanel(
  selectInput("select1", label = h4("Setor econômico"), 
    choices = empresas$`SETOR ECONOMICO`, selected = 1),
...
"Alguns ativos nao sao avaliados com a metodologia de Bazin. Por esse motivo, vai aparecer
...
var1.choice <- reactive({
                 status() %>% 
                 filter(`SETOR ECONOMICO` == input$select1) %>%
                 pull(SUBSETOR)
                  })
...
"Nota: Os dados estão filtrados para ticker com valores entre R$ 0 e R$ 1000.",
"Alguns ativos nao sao avaliados com a metodologia de Gordon. Por esse motivo, vai aparecer
a mensagem 'No rows found' para alguns setores.")
)
...
var2.choice <- reactive({
                 status() %>% 
                 filter(`SETOR ECONOMICO` == input$select3) %>%
                 pull(SUBSETOR)
                  })
...
"Nota: Os dados estao filtrados para ticker com valores entre R$ 0 e R$ 1000.",
"Alguns ativos nao sao avaliados com a metodologia de Graham. Por esse motivo, vai aparecer
a mensagem 'No rows found' para alguns setores.")
)
...
var3.choice <- reactive({
                 status() %>% 
                 filter(`SETOR ECONOMICO` == input$select5) %>%
                 pull(SUBSETOR)
                  })
              ...
              columns = list(
                ...
                `DESCONTO GRAHAM %` = colDef(
                                             ...
                                             cell = data_bars_pos_neg(., 
                                                       colors = c("#ff3030", "#1e90ff"))),
                `PEG Ratio` = colDef(name = "PEG ratio", align = "center"),
                `MÉDIA DE CRESCIMENTO %` = colDef(name = "Media de crescimento (%)", align = "center", 
                                                  cell = data_bars_pos_neg(., 
                                                       colors = c("#ff3030", "#1e90ff")))))
})
```
...
renderPlotly({
proventos_function(input$ativo6) %>% 
    ggplot(aes(x =  `Mes`, y = Valor, fill = Tipo)) +
    geom_bar(stat = "identity") +
    scale_y_continuous(name = "Valor por 1 acao (R$)") +
    scale_fill_viridis_d(option = "B") +
    labs(x = "", fill = "") +
    theme_test() +
    theme(axis.text.x = element_text(angle = 90),
